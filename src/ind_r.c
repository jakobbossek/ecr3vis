 ...
 * Utility functions:
 */
static double weighted_sum_utility(
  const double *wv, const double *x,
...
static double tchebycheff_utility(
  const double *wv, const double *x,
  const double *ideal, const double *nadir,
  const size_t nobjs) {
...
static double augmented_tchebycheff_utility(
  const double *wv, const double *x,
  const double *ideal, const double *nadir,
  const size_t nobjs,
  const double rho) {
...
size_t choose(int r, int k) {
  ...
  if (lower > upper) {
    int tmp = lower; lower = upper; upper = tmp;
  }
...
static double unary_tchebycheff_utility(
  const double *data,
...
SEXP do_unary_r2_ind(SEXP s_data, SEXP s_weights, SEXP s_ideal) {
  ...
  for(int i = 0; i < n_weights; ++i) {
    ...
    for (int j = 0; j < n_data; ++j) {
      ...
      const double utility = unary_tchebycheff_utility(current_data, current_weights, ideal, n_objectives);
