 ...
 */
static int get_dominance(double *points, R_len_t col1, R_len_t col2, R_len_t dim) {
  int dom1 = 0, dom2 = 0;
 ...
 * @return [logical(n)]
 */
SEXP dominated_c(SEXP r_points) {
  ...
  if(n_points > 1){
    ...
    for (int i = 0; i < n_points; ++i) {
      ...
      for (int j = (i + 1); j < n_points; ++j) {
        ...
        int dominance = get_dominance(c_points, i, j, dim);
        if (dominance > 0) {
          dominated[j] = TRUE;
        } else if (dominance < 0) {
          dominated[i] = TRUE;
...
SEXP nds_c(SEXP r_points) {
  ...
  // we want to return the "get_dominance counter" as well. Since
  ...
  while (n_sorted < n_points) {
    ...
    for (int i = 0; i < n_points; ++i) {
      ...
      for (int j = 0; j < n_points; ++j) {
        //FIXME: replace with non-redundant get_dominance check
        if (get_dominance(c_points, i, j, dim) > 0) { // i dominates j
          --c_domcounter[j];
